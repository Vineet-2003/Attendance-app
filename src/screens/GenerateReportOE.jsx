...
import { ScrollView, StyleSheet, Text, View, Button, Alert, Platform, TouchableOpacity } from 'react-native';
...
const GenerateReportOE = ({ route }) => {
  ...
  const calculateAttendanceData = useCallback(() => {
    ...
    return studentData.map((student, index) => {
      ...
      dates.forEach(date => {
        ...
        if (isPresent) {attendedDays++;}
      });

      const percentage = totalDays === 0 ? 0 : ((attendedDays / totalDays) * 100).toFixed(2);

      return {
        Department: student.Department,
        RollNumber: student.RollNumber,
        Name: student.Name,
        ...Object.fromEntries(attendanceRecords.map((record, i) => [`${dates[i]}`, record])),
  ...
  const saveExcel = async () => {
    ...
    if (Platform.OS === 'ios') {
      ...
      Alert.alert('Unsupported Platform', 'Saving Excel files is not supported on this platform.', [{ text: 'OK' }]);
      return;
    }
    ...
    const header = ['Department', 'Roll Number', 'Name', ...formattedDates, 'Total Attendance'];
    const data = displayData.map(student => ({
      'Department': student.Department,
      'Roll Number': student.RollNumber,
      'Name': student.Name,
      ...Object.fromEntries(formattedDates.map((date, i) => [formattedDates[i], student[dates[i]]])), // Map formatted dates to student data
    ...
    try {
      ...
      Alert.alert(
        'Completed',
        `Excel file saved successfully at ${path}`,
        [
          { text: 'OK' },
          {
            text: 'Cancel',
  ...
  return (
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.buttonContainer}>
        <Button title="All" onPress={() => setDisplayData(calculateAttendanceData())} />
        <Button title=">= 75%" onPress={() => filterData(75, 100)} />
        <Button title="65% - 74%" onPress={() => filterData(65, 74)} />
        <Button title="50% - 64%" onPress={() => filterData(50, 64)} />
        <Button title="< 50%" onPress={() => filterData(0, 49)} />
      </View>
          ...
          displayData.map((student, index) => (
            <TouchableOpacity key={index} onPress={() => showModal(student)}>
                <View style={styles.row}>
                  ...
                  <Text style={styles.rowText}>{student.Department}</Text>
